...
export const useSendInvitations = ({ poolId, poolName, emailMessage }: UseSendInvitationsProps) => {
  ...
  const prepareInvitations = (emailList: string[]): InvitationData[] => {
    return emailList.map(recipientEmail => {
      const link = generateInvitationLink(poolId, recipientEmail);
  ...
  const copyToClipboard = async (emailList: string[]): Promise<boolean> => {
    const invitationsData = prepareInvitations(emailList);
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  const openGmail = (emailList: string[]): boolean => {
    const invitationsData = prepareInvitations(emailList);
  ...
  const openMailClient = (emailList: string[]): boolean => {
    const invitationsData = prepareInvitations(emailList);
  ...
  const downloadAsFile = (emailList: string[]): boolean => {
    const invitationsData = prepareInvitations(emailList);
